...
# ------------------------------------------------------------------------------------------------------------
# this file depends on https://github.com/JonasFeron/PythonConnect 
...
def main_json_example(data_lines):
    """
    ...
    # 1) retrieve data
    dataString = data_lines[0] #data is contained in only one line in DataFile.txt
        
    twinData = json.loads(dataString, object_hook = d.ToTwinDataObject)   # the Data read from the data file.txt are stored in TwinData object (identical in python an C#)
    ...
    outputDct = json.dumps(twinResult, cls=r.TwinResultEncoder, ensure_ascii=False)  # , indent="\t"
    return outputDct
...
if __name__ == '__main__':  
    mainhelper.execute(main_json_example, sys.argv)
